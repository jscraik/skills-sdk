...
```bash
skills-sdk maintain-entrypoint /path/to/source/example/SKILL.md /path/to/runtime/example/SKILL.md --backup-root /path/to/backups --expected-source <candidate-sha256> --expected-current <approved-original-sha256>
```
...
source and target and add `--supporting-document`. The source directory must
still contain a valid identifying `SKILL.md`; the supplied hashes bind the
document itself. Nested references, scripts, missing files, and package-wide
installation are outside this maintenance route.
...
stages replacement bytes, and preserves the prior inode through an exclusive
hard-link backup before atomic publication. The adapter preserves target mode
bits and syncs the backup directory before publication. It does not preserve
...
a failure after publication may be indeterminate, with both replacement and
backup retained. Inspect them before attempting recovery. This is not a lock
