...
def _provider(*, model_id: str = "synthetic-model") -> ProviderIdentity:
...
def _observations_v2(
    scenario_set: ScenarioSetV2, *, digest: str = "b" * 64, provider: ProviderIdentity | None = None
...
def test_provider_model_and_draft_schema_reject_credential_components() -> None:
    payload = _provider().model_dump(mode="json")
    payload["model_id"] = "model:sk-live-secret"
