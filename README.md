...
The wrapper checks generator-managed schema drift, Ruff, the full pytest suite,
the source and wheel build, and `git diff --check`. The hand-maintained
`receipt-base.v1.schema.json`, `blocker.v1.schema.json`, and
`package-identity.v1.schema.json` resources are not regenerated by that
script; keep their direct Draft 2020-12/schema contract checks in the same
change. For any schema change, inspect both the generated and hand-maintained
resources and keep schema, behavior, and compatibility tests together. Read
[`CODESTYLE.md`](CODESTYLE.md) for implementation style and
